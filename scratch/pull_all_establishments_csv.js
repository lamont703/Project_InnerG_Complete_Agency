...
async function pullAllEstablishments() {
  ...
  const outputPath = path.join(__dirname, '../public/2026 Texas Barber and Beauty Salons.csv');
  
  // Write header to output file
  fs.writeFileSync(outputPath, CSV_HEADER, 'utf-8');
  
  console.log('Initiating bulk download of all "Full Service Establishment" records from live TDLR API...');
  console.log('Target file: public/2026 Texas Barber and Beauty Salons.csv\n');
  ...
  while (hasMore) {
    ...
    const query = {
      query: "SELECT * WHERE `license_type` = 'Full Service Establishment'",
      page: {
        pageNumber: pageNum,
        pageSize: pageSize
      }
  ...
  console.log(`Output: public/2026 Texas Barber and Beauty Salons.csv`);
}

pullAllEstablishments();
