...
def full_reconstruction():
    ...
    # 2. Parse ALL raw board data for institutional rates first
    school_stats = defaultdict(lambda: {'pass': 0, 'total': 0})
    processed_students_for_stats = set() # To avoid double counting duplicates in raw data
    
    # 3. Parse raw data for students
    all_students = []
    ...
    with open(raw_data_path, 'r') as f:
        for line in f:
            ...
            if match:
                ...
                # Clean student name (remove trailing TX if present)
                student_name = student_name_raw
                if student_name.endswith(" TX"):
                    student_name = student_name[:-3].strip()
                
                # Institutional stats calculation
                # Use a unique key to avoid double counting if the same student/code/test/date appears twice
    ...
    # 5. Add pass rates to student records
