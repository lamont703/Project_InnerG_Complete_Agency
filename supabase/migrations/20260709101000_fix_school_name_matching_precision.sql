-- Same precision bug as find_student_exam_record, found via live testing:
-- "Quality Barber College" pulled in "360 Barber College" and "Alamo
-- City Barber College" — completely unrelated schools, not a genuine
-- multi-campus collision like "Milan Institute" — because "Barber
-- College" is a generic, recurring phrase across many real institutions,
-- and the old threshold (token_matches > 0 OR trgm_sim > 0.25) let a
-- 1-of-3-token partial match through unchecked. Applying the same fix:
-- require matching at least min(2, query token count) tokens for multi-
-- word queries, trigram fallback restricted to single-word queries only.
...
AS $$
  ...
  WHERE scored.token_matches >= LEAST(2, qtc.n)
...
AS $$
  ...
  best_schools AS (
    ...
    WHERE ss.token_matches >= LEAST(2, qtc.n)
