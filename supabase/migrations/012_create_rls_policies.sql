...
-- developer: read-only, only clients assigned via developer_client_access
CREATE POLICY clients_developer_read ON public.clients
  FOR SELECT USING (
    auth_role() = 'developer' AND
    id IN (
      SELECT client_id FROM public.developer_client_access
      WHERE developer_id = auth.uid()
    )
  );

-- client_admin, client_viewer: read their own client record only
...
-- developer: read-only, only projects whose client is in their portfolio
CREATE POLICY projects_developer_read ON public.projects
  FOR SELECT USING (
    auth_role() = 'developer' AND
    client_id IN (
      SELECT client_id FROM public.developer_client_access
...
-- client users: read-only, only projects in project_user_access
CREATE POLICY projects_client_read ON public.projects
  FOR SELECT USING (
    auth_role() IN ('client_admin', 'client_viewer') AND
    id IN (
      SELECT project_id FROM public.project_user_access
      WHERE user_id = auth.uid()
    )
  );

-- ── SENSITIVE TABLES (Inner G team only) ─────────────────────

-- client_db_connections: Inner G team only (never expose connection URLs to clients)
CREATE POLICY client_db_connections_team_only ON public.client_db_connections
  FOR ALL USING (is_inner_g_team());

-- invite_links: only super_admin and developers who created the link
CREATE POLICY invite_links_creator ON public.invite_links
  FOR SELECT USING (
    auth_role() = 'super_admin' OR
    invited_by = auth.uid()
  );
CREATE POLICY invite_links_super_admin_write ON public.invite_links
  FOR ALL USING (auth_role() = 'super_admin');

-- growth_audit_leads: Inner G team only
CREATE POLICY growth_audit_leads_team_only ON public.growth_audit_leads
  FOR ALL USING (is_inner_g_team());

-- document_embeddings: Inner G team only (RAG vectors are internal)
CREATE POLICY document_embeddings_team_only ON public.document_embeddings
  FOR ALL USING (is_inner_g_team());

-- embedding_jobs: Inner G team only
CREATE POLICY embedding_jobs_team_only ON public.embedding_jobs
  FOR ALL USING (is_inner_g_team());

-- ── PROJECT-SCOPED DATA: Campaigns, Metrics, Signals, Activity ─

-- Shared helper: check if user can access a project
-- (DRY pattern reused across multiple table policies)
...
-- ai_signals: all users with project access can read; Inner G team can write
CREATE POLICY ai_signals_read ON public.ai_signals
  FOR SELECT USING (can_access_project(project_id));
CREATE POLICY ai_signals_team_write ON public.ai_signals
  FOR ALL USING (is_inner_g_team());

-- campaigns: all users read; Inner G team write
CREATE POLICY campaigns_read ON public.campaigns
  FOR SELECT USING (can_access_project(project_id));
CREATE POLICY campaigns_team_write ON public.campaigns
  FOR ALL USING (is_inner_g_team());

-- campaign_metrics: read only for project users
CREATE POLICY campaign_metrics_read ON public.campaign_metrics
  FOR SELECT USING (
    can_access_project((SELECT project_id FROM public.campaigns WHERE id = campaign_id))
  );
CREATE POLICY campaign_metrics_team_write ON public.campaign_metrics
  FOR ALL USING (is_inner_g_team());

-- activity_log: read for project users; system/team write
CREATE POLICY activity_log_read ON public.activity_log
  FOR SELECT USING (can_access_project(project_id));
CREATE POLICY activity_log_team_write ON public.activity_log
  FOR INSERT USING (is_inner_g_team());

-- system_connections: read for project users; team write
CREATE POLICY system_connections_read ON public.system_connections
  FOR SELECT USING (can_access_project(project_id));
CREATE POLICY system_connections_team_write ON public.system_connections
  FOR ALL USING (is_inner_g_team());
