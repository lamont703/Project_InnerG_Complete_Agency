-- "Qualified Visitor" (used here and in get_entity_type_ctr) is defined as
-- "did anything beyond just viewing the page" via event_name != 'page_view'.
-- Adding a new automatic page_load_timing ping (fired on every page load,
-- no user action involved) would otherwise make every visitor "qualified"
-- just for loading a page, silently inflating Qualified Visitors, Category
-- Views, and entity CTR denominators with zero real behavior change. Both
-- functions are otherwise byte-identical to their prior versions — only the
...
BEGIN
  ...
  FROM (
    ...
    GROUP BY visitor_id
    HAVING
      COUNT(*) FILTER (WHERE event_name NOT IN ('page_view', 'page_load_timing')) > 0
  ...
  -- 7. Category Views — scoped to qualified visitors only.
...
BEGIN
  ...
  WITH qualified_visitors AS (
    ...
    HAVING
      COUNT(*) FILTER (WHERE pe.event_name NOT IN ('page_view', 'page_load_timing')) > 0
