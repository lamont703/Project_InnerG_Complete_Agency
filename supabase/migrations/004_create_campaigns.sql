...
-- One row per campaign per day — generated by generate-daily-snapshot Edge Function
-- or pulled from client_db_connections (KPI Aggregation)
...
CREATE INDEX idx_campaign_metrics_campaign_date ON public.campaign_metrics(campaign_id, snapshot_date DESC);
...
CREATE INDEX idx_funnel_stages_campaign ON public.funnel_stages(campaign_id, position);
...
CREATE INDEX idx_funnel_events_stage_date ON public.funnel_events(funnel_stage_id, snapshot_date DESC);
