-- "Returning Users" previously used two different definitions depending on
-- whether a timeframe cutoff was set:
--   - With a cutoff: visitors active in-window AND with any activity before
--     the cutoff. Not monotonic — someone who first showed up 10 days ago
--     counts as returning in the 7-day view (they have history before the
--     7-day cutoff) but not in the 30-day view (nothing exists before the
--     30-day cutoff, since they only started 10 days ago). This let a
--     *shorter* window show a *higher* count than a longer one that should
--     be its superset.
--   - Without a cutoff ("All Time"): visitors with activity on 2+ distinct
--     calendar dates, ever — a completely different rule.
--
-- Unified on the "All Time" rule (2+ distinct calendar days of activity),
-- scoped to the selected window in both cases. This is monotonic by
-- construction: widening the window can only add days to a visitor's
-- activity, never remove them, so a visitor who qualifies in a shorter
-- window still qualifies in every window that contains it.
