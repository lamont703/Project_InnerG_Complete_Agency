-- Converting to plpgsql (for the DECLARE block) made the RETURNS TABLE
-- output names (entity_type, visits, outbound_clicks, ctr) implicit
-- variables in scope for the whole function body, which collided with
-- same-named CTE columns and threw "column reference is ambiguous" on
-- every call. Renaming the CTE-internal column so it can't collide with
...
BEGIN
  ...
  visit_agg AS (
    SELECT pv.etype, COUNT(*) AS visit_count
