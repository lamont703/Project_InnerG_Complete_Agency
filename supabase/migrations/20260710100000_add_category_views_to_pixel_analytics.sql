-- Adds a per-entity-category visitor breakdown (shops, salons, barbers,
-- cosmetologists, schools, stores, events, insights, tools, other/home) to
-- the existing pixel analytics summary RPC, using the same clean_url
-- extraction and page_view scoping already used by the "Top Pages" block
-- above it — categorization is just a path-prefix bucketing of the same
-- underlying data, not a new tracking mechanism.
...
BEGIN
  ...
  -- 7. Category Views — page views + unique visitors per entity/page
  -- category, bucketed by the same clean_url path prefix used above.
  -- Order of WHEN clauses matters: /events must be checked before the
  -- catch-all so the events *directory* itself still counts as Events, not
  -- Other, and /tools is checked last since some tool paths (exam prep,
  -- practice deck) are nested under otherwise-generic-looking prefixes.
