...
DROP FUNCTION IF EXISTS public.search_barbershops_ranked(text, boolean, text, int, int, vector);
...
BEGIN
  ...
  WITH search_results AS (
    SELECT 
      ...
      -- Calculate semantic similarity if vector provided
      (CASE 
  ...
  SELECT 
    ...
    s.google_images,
    (
      -- Semantic Bonus (Max ~200 points for a perfect match)
  ...
  WHERE 
    ...
    AND (
      ...
      OR s.shop_name ILIKE '%' || query_text || '%' 
      OR s.city ILIKE '%' || query_text || '%'
      -- If semantic similarity is decent (> 0.5), it bypasses the strict text matching
