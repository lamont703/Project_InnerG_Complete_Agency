-- ChatGPT strips the Referer header entirely on outbound citation links
-- (rel="noreferrer"), so the existing referrer-domain parsing below would
-- always show these as "Direct / Unknown" — indistinguishable from a
-- bookmark or typed-in URL. Since mid-2025 ChatGPT instead appends
-- utm_source=chatgpt.com as a query param on citation links, which survives
-- regardless of referrer policy — this labels those visits explicitly,
-- same pattern already used for Email Campaign detection below.
-- Perplexity needs no special case here: it does pass a real referrer, so
-- it's already correctly captured by the generic domain-parsing branch.
...
BEGIN
  ...
  -- 5. Top Referrers — ChatGPT's utm_source checked before falling back to
  -- the generic referrer-domain parse, since ChatGPT's stripped Referer
  -- header would otherwise always fall through to "Direct / Unknown".
  ...
  FROM (
    SELECT
      CASE
        ...
        WHEN page_url ILIKE '%utm_source=chatgpt.com%' THEN 'ChatGPT (AI Citation)'
        ELSE COALESCE(
