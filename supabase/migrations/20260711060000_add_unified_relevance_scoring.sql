-- Purely additive change to all 8 ranked-search RPCs: adds `base_relevance`
-- (0-100, semantic+keyword only, same formula/scale in every category) and
-- `quality_bonus` (0-0.40 capped fraction, each category's own non-hiring
-- quality signals re-expressed as a bounded multiplier instead of a flat
-- add) so the "All" tab can blend heterogeneous categories into one truly
-- ranked list. Shop also gets `hiring_bonus` (0 or 0.35) split out
-- separately, since hiring_need is a job-seeking-context signal, not a
-- general relevance/quality one — the app decides how much weight it gets
-- based on detected search intent, not this RPC.
--
-- Existing WHERE/ORDER BY/match_score/trust_score are left completely
-- untouched, so every dedicated tab (Schools, Barbershops, Salons, etc.)
-- keeps its exact current behavior. Only the new columns are added.
--
-- One incidental fix: shop's trust_score scales semantic similarity by
-- 200 (0.80 weight => max 160) while every other category scales it by
-- 100 (max 80) — the only reason shops were ever comparable to nothing
-- but themselves. base_relevance uses the same ×100×0.80 scale as
-- everyone else so it's actually comparable across categories.
--
-- Also restores a claimed-listing bonus into quality_bonus for shops —
-- 20260708000000 added a +150 claimed_at bonus to trust_score, but
-- 20260710070000's DROP+CREATE (for the slug column) silently dropped it
-- when copying the function forward. Folding it back in here.
