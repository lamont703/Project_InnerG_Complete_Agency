-- Surface the scraped Booksy photo/rating in barber search results so the
-- search card matches the new standard barber profile page.
-- Return type is changing (new output columns), so the old signature must be dropped first.
