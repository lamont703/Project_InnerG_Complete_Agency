...
export function OmniChannelStream({ projectSlug, title = "Omni-Channel Stream", showMetricsGrid = false }: OmniChannelStreamProps) {
    ...
    const config = funnelConfig || {
        sources: [
            ...
            { id: 'tw', enabled: true, metric: 'impressions', label: "X / Twitter", subLabel: "Pulse Traffic", color: "bg-zinc-800", hex: "#000000" },
    ...
    const funnelData = {
        sources: config.sources.map((s: any) => {
            ...
            const displayVal = Number(rawVal).toLocaleString();
        ...
        engagement: {
            ...
            rawValue: (
                (config.engagement.metrics.find((m: any) => m.id === 'likes')?.enabled ? (
                    (youtubeMetrics?.likes || 0) + (linkedinMetrics?.likes || 0) + (instagramMetrics?.likes || 0) + (tiktokMetrics?.videoLikes || 0) + (facebookMetrics?.likes || 0) + (twitterMetrics?.likes || 0)
                ) : 0) +
                (config.engagement.metrics.find((m: any) => m.id === 'comments')?.enabled ? (
                    (youtubeMetrics?.comments || 0) + (linkedinMetrics?.comments || 0) + (instagramMetrics?.comments || 0) + (tiktokMetrics?.videoComments || 0) + (facebookMetrics?.comments || 0) + (twitterMetrics?.replies || 0)
                ) : 0) +
                (config.engagement.metrics.find((m: any) => m.id === 'shares')?.enabled ? (
                    (linkedinMetrics?.shares || 0) + (tiktokMetrics?.videoShares || 0) + (twitterMetrics?.retweets || 0)
                ) : 0) +
            ...
            metrics: config.engagement.metrics.filter((m: any) => m.enabled).map((m: any) => {
                let val = "0";
                if (m.id === 'likes') val = ((youtubeMetrics?.likes || 0) + (linkedinMetrics?.likes || 0) + (instagramMetrics?.likes || 0) + (tiktokMetrics?.videoLikes || 0) + (facebookMetrics?.likes || 0) + (twitterMetrics?.likes || 0)).toLocaleString();
                if (m.id === 'comments') val = ((youtubeMetrics?.comments || 0) + (linkedinMetrics?.comments || 0) + (instagramMetrics?.comments || 0) + (tiktokMetrics?.videoComments || 0) + (facebookMetrics?.comments || 0) + (twitterMetrics?.replies || 0)).toLocaleString();
                if (m.id === 'shares') val = ((linkedinMetrics?.shares || 0) + (tiktokMetrics?.videoShares || 0) + (twitterMetrics?.retweets || 0)).toLocaleString();
            ...
            <div className="absolute top-6 right-10 flex items-center gap-6 z-20">
                <div className="flex flex-col items-end">
                    ...
                    <span className="text-sm font-black text-white tracking-tight">
                        {((youtubeMetrics?.views || 0) + (tiktokMetrics?.totalViews || 0) + (linkedinMetrics?.views || 0) + (instagramMetrics?.reach || 0) + (twitterMetrics?.impressions || 0)).toLocaleString()}
                    </span>
                </div>
            </div>
