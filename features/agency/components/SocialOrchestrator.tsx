...
import { Sparkles, Send, Edit3, Github, FileText, CheckCircle2, Loader2, Linkedin, Instagram } from "lucide-react"
                ...
                {drafts.length === 0 ? (
                    ...
                    drafts.map((draft) => (
                        ...
                        >
                                ...
                                <div className="flex items-center gap-3">
                                    <div className="h-8 w-8 rounded-lg bg-muted/20 flex items-center justify-center border border-border">
                                        {draft.platform === 'linkedin' ? (
                                            ...
                                            <Instagram className="h-4 w-4 text-pink-400" /> // Using insta icon for tiktok as fallback if no tiktok icon
                                        ) : (
                            ...
                            <div className="mb-4">
                                <p className="text-xs text-foreground/90 leading-relaxed font-medium line-clamp-3 italic">
                                    "{draft.content_text}"
                                </p>
                            </div>

                            {draft.ai_reasoning && (
                                <div className="p-3 rounded-xl bg-violet-500/5 border border-violet-500/10 mb-5">
                                ...
                                <div className="flex items-center gap-3">
                                    {draft.source_type !== 'manual' && (
                                        <div className="flex items-center gap-1 text-[9px] font-bold text-muted-foreground/60 uppercase tracking-widest">
                                            {draft.source_type === 'github' ? <Github className="h-3 w-3" /> : <FileText className="h-3 w-3" />}
                                            Context: {draft.source_type}
                                        </div>
                                    )}
                                </div>
