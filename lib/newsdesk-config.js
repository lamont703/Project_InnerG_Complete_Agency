...
const NEWSDESK = {
  ...
  visuals: {
     ...
     */
    /*
     * A TIGHT CROP OF A NEWS SCREENSHOT DOES NOT WORK. Tested on two different
     * articles and it failed the same way both times: phys.org and CBS both set
     * their headline at full page width, so any zoom past about 1.15 clips words
     * off both sides and the headline stops being readable — which is the only
     * reason it is on screen.
     *
     * So there is no generic "second view" of an article page. When a headline
     * segment runs long, the fix is EDITORIAL not visual: move a sentence into a
     * b-roll segment, which is index-stable and therefore free, rather than
     * hunting for a crop. See the note on shotLengths in render_news_short.js.
     *
     * The chart crops below are the exception that proves it — they work because
     * that screenshot happened to contain a FIGURE, which is a separate object
     * with its own margins. Most articles have no such thing.
     */
 ...
 */
/*
 * THE CAP IN SECONDS, WHICH IS THE UNIT THE SCRIPT IS ACTUALLY WRITTEN IN.
 *
 * A dollar cap cannot be obeyed by someone writing sentences — "keep avatar
 * seconds low" is not a budget, and a model given only that wrote a spec at
 * $1.89 against a $1.50 gate. Seconds are checkable while writing: at 175 wpm,
 * this many seconds is a countable number of words.
 */
function maxAvatarSecs(profile) {
  return Math.floor((profile || NEWSDESK).budgetUsd / AVATAR_PER_SEC);
}
...
module.exports = { NEWSDESK, HOTTAKE_SEGMENTED, REACTION, PROFILES, AVATAR_PER_SEC, WORDS_PER_MIN, estimateAvatarUsd, withinBudget, maxAvatarSecs };
