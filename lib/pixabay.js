...
async function searchVideos(query, opts = {}) {
  ...
  /*
   * FILM, NOT ANIMATION, AND THIS IS A DEFAULT NOT AN OPTION. Pixabay mixes
   * illustrated and animated clips in with live footage and nothing in the tags
   * distinguishes them — "bank loan manager" returned a crude black-and-white
   * line drawing of stacked coins, correctly tagged and completely unusable
   * next to a real person talking. video_type=film is the only thing that
   * separates them. Pass "all" deliberately if animation is ever wanted.
   */
  params.set("video_type", opts.videoType ?? "film");
...
function pickBest(hits, opts = {}) {
  ...
  /*
   * Words that carry no visual meaning are dropped. "man", "person" and "people"
   * are on almost every clip of a human being, so ranking on them is ranking on
   * nothing — and if they are the ONLY words that match, the clip is a
  ...
  (hits ?? []).forEach((h, position) => {
    ...
    /*
     * WHOLE TAG WORDS, NOT SUBSTRINGS. This was `tags.includes(word)`, and a
     * substring test on short words matches almost anything: "man" is inside
     * "human", "woman", "romantic", "germany" and "manual". The query
     * "man signing paper" returned a muddy creek — it matched on "man" hiding
     * inside an unrelated tag, and every other check passed it.
     *
     * Tags arrive comma-separated and may be multi-word ("human hand"), so they
     * are split on both.
     */
    const tagWords = new Set(
      String(h.tags ?? "").toLowerCase().split(/[,\s]+/).map((t) => t.trim()).filter(Boolean)
    );
    const matched = want.filter((w) => tagWords.has(w)).length;
    /*
     * ONE WORD OUT OF THREE IS A COINCIDENCE, NOT AN ANSWER. Requiring merely
     * "some overlap" let "man signing paper" match a clip tagged "paper ship" —
     * a paper boat floating down a river. Whole-word matching was already on;
     * the match was real and the clip was still nonsense, because a lone
     * concrete noun says nothing about what the footage shows.
     *
     * A majority of the meaningful words must land. For a two-word query that
     * means both, which is the intended strictness: "signing paper" is a scene,
     * "paper" is a material.
     */
    const mustMatch = Math.max(1, Math.ceil(want.length * 0.6));
    if (want.length && matched < mustMatch) return;
