...
/**
 * The source tag the redirect STAMPS ON, so attribution cannot be forgotten.
 *
 * The pixel never fires on /youtube/<slug> — a 307 serves no HTML, so no script
 * runs and nothing is recorded there. The destination pageview is the only
 * evidence the visit happened at all, which makes the query string the only
 * carrier of where it came from. Leaving that to whoever writes the description
 * means the one run that matters is the one where it was forgotten, so the
 * redirect adds it rather than the human.
 *
 * `?v=<video>` in the description is additive on top of this: Next.js merges
 * the request's query into the destination's, so a bare link still records the
 * channel and audience, and a tagged link also records which video.
 */
export const sourceTag = (slug) => `youtube-${slug}`;

/** The redirect rules, in the shape next.config.mjs wants. */
export const youtubeLinkRedirects = () =>
  YOUTUBE_LINKS.map(({ slug, destination }) => ({
    source: `/youtube/${slug}`,
    destination: `${destination}?src=${sourceTag(slug)}`,
