 ...
 * @param {{tags: string[], minSeconds?: number, limit?: number, exclude?: string[],
 *          minScore?: number, includeStylized?: boolean}} o
...
async function findClips(db, o) {
  ...
  return rankClips(data, { ...o, tags });
}

/**
 * Tags that mark footage as DRAWN rather than filmed.
 *
 * WHY THIS LIST IS A GUARD AND NOT A LABEL. The library held only photoreal
 * footage until the two short films went in — 52 clips of hand-drawn cartoon
 * line art, every one of them legitimately tagged `barbershop`, `barber`,
 * `chair`, `cape`, because that is what is in them. A News Desk asking for
 * "barbershop chair" then scores 2 on a pencil sketch and puts it in a video
 * about a real news story, which is the exact failure add_broll already calls
 * out: showing the wrong picture is worse than showing the speaker.
 *
 * So stylized clips are EXCLUDED BY DEFAULT and must be asked for. Asking is
 * either `includeStylized: true` or simply naming one of these tags in the
 * search — if you search for "cartoon", you meant it.
 */
const STYLIZED_TAGS = ["cartoon", "sketch", "line-art", "animation"];

/**
 * The scoring half of findClips, split out so it can be tested without a
 * database. Pure: rows in, ranked rows out.
 *
 * @param {Array<Object>} rows
 * @param {{tags: string[], limit?: number, exclude?: string[], minScore?: number, includeStylized?: boolean}} o
 */
function rankClips(rows, o) {
  const tags = (o.tags || []).map((t) => String(t).toLowerCase().trim()).filter(Boolean);
  const reaches = (tag, word) =>
    tag === word ||
    (word.length >= 4 && tag.startsWith(word)) ||
  ...
  // Naming a stylized tag in the search IS the opt-in, so a caller after the
  // cartoon material does not also have to know about the flag.
  const wantsStylized = o.includeStylized === true || tags.some((t) => STYLIZED_TAGS.includes(t));

  return (rows || [])
    .filter((r) => !exclude.has(r.id))
    .filter((r) => wantsStylized || !(r.tags || []).some((t) => STYLIZED_TAGS.includes(t)))
    .map((r) => ({ ...r, _score: (r.tags || []).filter((t) => tags.some((w) => reaches(t, w))).length }))
    .filter((r) => r._score >= minScore)
    .sort((a, b) => b._score - a._score || (a.use_count ?? 0) - (b.use_count ?? 0))
    .slice(0, o.limit ?? 10);
}
...
module.exports = { findClips, rankClips, recordClip, markUsed, STYLIZED_TAGS };
