 ...
 */
function buildPrompt({ script, words, joins, duration, tracks, libraryTags = [] }) {
...
WRITE QUERIES OUT OF THIS VOCABULARY. These are the tags on the footage we
actually own — there is no stock search behind this any more, only these clips:
${(libraryTags ?? []).join(", ") || "(none loaded)"}

   A query needs at least TWO of these words to match anything, so a moment you
   can only describe with one of them will get NO cutaway and the speaker stays
   on screen. That is the correct outcome, not a failure — showing the wrong
   picture is worse. If a beat cannot be said in this vocabulary, spend your
   cutaways on a beat that can.

   This list is the whole library. Do not invent a subject that is not in it:
   "map pin navigation" has no footage, and asking for it once put a ringing
   desk phone on screen to illustrate a map.
