/**
 * Turning "cut these bits out" into "keep these bits".
 *
...
export function normaliseCuts(cuts: Range[], duration: number): Range[] {
  const clean = cuts
...
export function keepRanges(cuts: Range[], duration: number): Range[] {
 ...
 */
export function selectFilter(keep: Range[]): { video: string; audio: string } | null {
