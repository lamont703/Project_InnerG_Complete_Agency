...
async function pullGooglePlacesBeautySupplyStores() {
  ...
  console.log('--- Google Places API (New) v1 Houston Beauty Supply Store Extractor ---\n');

  // Comprehensive list of major Houston zip codes (same sweep used for barbershops
  // and barber supply stores, so coverage stays consistent across all directories).
  const zipCodes = [
  ...
  // Two text queries per zip so we catch both "beauty supply" and "hair supply"
  // storefronts (many Houston stores brand themselves under either term).
  const searchTerms = ['beauty supply store', 'hair supply store'];
  ...
  console.log(`\n💾 Total unique Houston beauty supply stores compiled: ${allPlaces.size}`);
  ...
  // --- Dedup guard against the barber supply store table ---
  // Some storefronts (e.g. shops that sell both barber and beauty/hair
  // supplies) can be returned by both directories' Places queries. Per
  // product requirement, any place already tracked as a barber supply store
  // should NOT also be added to the beauty supply store table.
  console.log(`\n🔎 Checking for overlap with agent_barber_supply_store_leads...`);
  const existingBarberSupplyPlaceIds = new Set();
  {
    ...
    while (true) {
      ...
      if (error) {
        console.error('❌ Failed to fetch existing barber supply store place_ids:', error.message);
        break;
      }
      if (!data || data.length === 0) break;

      data.forEach(row => { if (row.place_id) existingBarberSupplyPlaceIds.add(row.place_id); });
      if (data.length < PAGE_SIZE) break;
      from += PAGE_SIZE;
    }
  }
  console.log(`   - Found ${existingBarberSupplyPlaceIds.size} existing barber supply store place_id(s) to exclude.`);

  let duplicateSkipCount = 0;
  const placesToImport = [];
  for (const [placeId, place] of allPlaces.entries()) {
    if (existingBarberSupplyPlaceIds.has(placeId)) {
      duplicateSkipCount++;
      continue;
    }
    placesToImport.push(place);
  }
  console.log(`   - Skipping ${duplicateSkipCount} store(s) already tracked as barber supply stores.`);

  console.log(`\n🚀 Starting Direct Upsert to Supabase (${placesToImport.length} store(s))...`);
  let successCount = 0;
  let errorCount = 0;
  ...
  console.log(`⏭️  Skipped (Duplicate of Barber Supply Store): ${duplicateSkipCount}`);
  console.log(`❌ Errors: ${errorCount}`);
  console.log("================================================\n");
}

pullGooglePlacesBeautySupplyStores();
