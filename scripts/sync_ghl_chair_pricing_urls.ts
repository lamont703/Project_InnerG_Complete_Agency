...
async function syncUrlsToGhl() {
  console.log("🚀 Starting GHL Contact Sync for Chair Pricing URLs...");

  // Fetch shops that have a contact_id AND a chair_pricing_tool_url
  const { data: shops, error: fetchError } = await supabase
    .from("agent_barbershop_leads")
    .select("id, shop_name, contact_id, chair_pricing_tool_url")
  ...
  if (!shops || shops.length === 0) {
    console.log("✅ No shops found that have both a contact_id and a chair_pricing_tool_url. Nothing to update.");
    Deno.exit(0);
  }
  ...
  for (const shop of shops) {
    try {
      const res = await fetch(`https://services.leadconnectorhq.com/contacts/${shop.contact_id}`, {
        ...
        body: JSON.stringify({
          customFields: [
            {
              id: CUSTOM_FIELD_ID,
              key: "contact.chair_pricing_tool_url",
              field_value: shop.chair_pricing_tool_url
            }
          ]
        })
      });
      ...
      if (!res.ok) {
        ...
        console.log(`✅ Synced GHL Contact for ${shop.shop_name} (${shop.chair_pricing_tool_url})`);
        successCount++;
      }
    } catch (err: any) {
