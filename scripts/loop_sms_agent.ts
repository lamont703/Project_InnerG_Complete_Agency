/**
 * scripts/loop_sms_agent.ts
 *
 * Runs the SMS Barbershop trigger in a continuous loop.
 */
...
console.log(`🚀 Starting SMS Agent Loop every ${INTERVAL_MINUTES} minutes.`);

async function runLoop() {
  while (true) {
    console.log(`\n======================================================`);
    console.log(`⏱️ Running SMS Agent at ${new Date().toISOString()}`);
    console.log(`======================================================`);
    ...
    try {
      ...
      if (typeof Deno.Command !== "undefined") {
        const command = new Deno.Command(Deno.execPath(), {
          args: ["run", "-A", "scripts/trigger_sms_barbershop_agent.ts", BATCH_SIZE],
          env: { AUTO_APPROVE: "1" },
          stdout: "inherit",
          stderr: "inherit",
        });
        ...
        const p = Deno.run({
          cmd: [Deno.execPath(), "run", "-A", "scripts/trigger_sms_barbershop_agent.ts", BATCH_SIZE],
          env: { AUTO_APPROVE: "1" },
        });
