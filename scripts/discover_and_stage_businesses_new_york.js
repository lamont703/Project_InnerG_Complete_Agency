// New York twin of scripts/discover_and_stage_businesses.js (the
...
// (looks for "NY"/"New York" instead of "TX"/"Texas"), and one
...
// it here would silently orphan every New York finding from all four —
...
// New York data; they process it exactly like Texas data, since as far
...
// chain with both a Texas and a New York location) automatically once
// New York rows start publishing.
//
...
//
// Three modes:
//   node scripts/discover_and_stage_businesses_new_york.js "Buffalo NY"
//     Manual — discover exactly the city you name.
//   node scripts/discover_and_stage_businesses_new_york.js
...
//     NEW YORK CHANGE / real caveat: Google Ads Agent's directives don't
//     currently carry a state field, only a bare city name — if that agent
//     is ever run for New York too, an ambiguously-named city could in
//     theory get picked up by both this script's auto mode and the Texas
//     script's auto mode. Not a concern today (no New York keyword
//     research has been run), but worth a second look before relying on
//     auto mode here once it has.
//   node scripts/discover_and_stage_businesses_new_york.js --all-cities
//     State-wide sweep — every city in NY_CITIES below, one after another,
...
// NEW YORK CHANGE — the Texas script's TX_CITIES was an established,
...
// New York's ~34 largest cities by population, covering New York City and its metro, Long Island, the Hudson
// Valley, the Capital Region, Central and Western New York, and the
// Southern Tier — not
...
const NY_CITIES = [
  ...
  'ithaca', 'poughkeepsie', 'jamestown', 'elmira', 'newburgh',
  'middletown', 'auburn', 'watertown', 'glen cove', 'saratoga springs',
  'kingston', 'peekskill', 'lockport', 'plattsburgh',
];
...
async function extractFullDetail(page, name, city) {
  ...
  const detail = await page.evaluate(() => {
    ...
    const addressLine = lines.find((l) => /\d/.test(l) && /(NY|New York)\b|\b\d{5}\b/.test(l) && l.length < 90 && !/^\(/.test(l));
...
async function discoverCity(browser, cityArg, cityLabel, candidateMap, candidatePhoneMap) {
  ...
  for (const table of ACTIVE_TABLES) {
    ...
    // name Set would incorrectly skip a real New York candidate that
...
async function run() {
  ...
  let targets;
  if (ALL_CITIES_MODE) {
    targets = NY_CITIES.map((c) => ({ cityArg: `${titleCase(c)} NY`, cityLabel: titleCase(c), sourceDirective: null }));
    console.log(`State-wide sweep mode: ${targets.length} New York cities queued.`);
    ...
    targets = [{ cityArg, cityLabel: cityArg.replace(/\s*NY$/i, '').trim(), sourceDirective: null }];
  } else {
    ...
    if (approved.length === 0) {
      ...
      console.log('Either approve one at /admin/agent-directives, or run manually: node scripts/discover_and_stage_businesses_new_york.js "City NY"');
      return;
    }
    targets = approved.map((d) => ({ cityArg: `${d.evidence.city} NY`, cityLabel: d.evidence.city, sourceDirective: d }));
