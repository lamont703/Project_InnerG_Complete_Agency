// California twin of scripts/discover_and_stage_businesses.js (the
...
// (looks for "CA"/"California" instead of "TX"/"Texas"), and one
...
// it here would silently orphan every California finding from all four —
...
// California data; they process it exactly like Texas data, since as far
...
// chain with both a Texas and a California location) automatically once
// California rows start publishing.
//
...
//
// Three modes:
//   node scripts/discover_and_stage_businesses_california.js "Sacramento CA"
//     Manual — discover exactly the city you name.
//   node scripts/discover_and_stage_businesses_california.js
...
//     CALIFORNIA CHANGE / real caveat: Google Ads Agent's directives don't
//     currently carry a state field, only a bare city name — if that agent
//     is ever run for California too, an ambiguously-named city could in
//     theory get picked up by both this script's auto mode and the Texas
//     script's auto mode. Not a concern today (no California keyword
//     research has been run), but worth a second look before relying on
//     auto mode here once it has.
//   node scripts/discover_and_stage_businesses_california.js --all-cities
//     State-wide sweep — every city in CA_CITIES below, one after another,
...
// CALIFORNIA CHANGE — the Texas script's TX_CITIES was an established,
...
// California's ~34 largest cities by population, covering Southern
// California, the Bay Area, the Central Valley, and the Sacramento
// region — not validated against real Google Ads/GSC keyword-demand
// research the way the Texas expansion decisions were. Worth the same
// "check real demand before treating this as final" pass the Texas
// city-by-city rollout got, rather than assuming every city on this list
// is actually worth discovering into.
const CA_CITIES = [
...
async function extractFullDetail(page, name, city) {
  ...
  const detail = await page.evaluate(() => {
    ...
    const addressLine = lines.find((l) => /\d/.test(l) && /(CA|California)\b|\b\d{5}\b/.test(l) && l.length < 90 && !/^\(/.test(l));
...
async function discoverCity(browser, cityArg, cityLabel, candidateMap, candidatePhoneMap) {
  ...
  for (const table of ACTIVE_TABLES) {
    ...
    // name Set would incorrectly skip a real California candidate that
...
async function run() {
  ...
  let targets;
  if (ALL_CITIES_MODE) {
    targets = CA_CITIES.map((c) => ({ cityArg: `${titleCase(c)} CA`, cityLabel: titleCase(c), sourceDirective: null }));
    console.log(`State-wide sweep mode: ${targets.length} California cities queued.`);
    ...
    targets = [{ cityArg, cityLabel: cityArg.replace(/\s*CA$/i, '').trim(), sourceDirective: null }];
  } else {
    ...
    if (approved.length === 0) {
      ...
      console.log('Either approve one at /admin/agent-directives, or run manually: node scripts/discover_and_stage_businesses_california.js "City CA"');
      return;
    }
    targets = approved.map((d) => ({ cityArg: `${d.evidence.city} CA`, cityLabel: d.evidence.city, sourceDirective: d }));
