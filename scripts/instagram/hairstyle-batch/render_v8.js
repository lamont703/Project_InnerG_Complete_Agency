/* Render the v8 hairstyle Reels — tie-ups, edges, faux locs and twists.
   Two men's, two women's, none of them run before.

   FIFTH NEAR-COPY OF THIS FILE. v3 through v8 differ only in the batch name;
   ...
   READS THE .fit.jpg, NOT THE RAW GRID, and that matters. The v8 grids come
...
fs.mkdirSync("experiments/hairstyle-reels-v8", { recursive: true });
fs.mkdirSync("experiments/hairstyle-covers-v8", { recursive: true });
for (const c of cfg) {
  const grid = `experiments/hairstyle-grids-v8/${c.k}.fit.jpg`;
  if (!fs.existsSync(grid)) { console.log(`MISSING grid ${grid}`); continue; }
  const out = `experiments/hairstyle-reels-v8/${c.k}.mp4`;
  const cover = `experiments/hairstyle-covers-v8/${c.k}.jpg`;
