/* Render the v6 hairstyle Reels — two men's, two women's, all styles the
   channel has never run. Skips anything already rendered so an interrupted run
   resumes; a Reel is 270 puppeteer frames and takes about four minutes.

   READS THE .fit.jpg, NOT THE RAW GRID, and that matters. The v6 grids come
...
fs.mkdirSync("experiments/hairstyle-reels-v6", { recursive: true });
fs.mkdirSync("experiments/hairstyle-covers-v6", { recursive: true });
for (const c of cfg) {
  const grid = `experiments/hairstyle-grids-v6/${c.k}.fit.jpg`;
  if (!fs.existsSync(grid)) { console.log(`MISSING grid ${grid}`); continue; }
  const out = `experiments/hairstyle-reels-v6/${c.k}.mp4`;
  const cover = `experiments/hairstyle-covers-v6/${c.k}.jpg`;
