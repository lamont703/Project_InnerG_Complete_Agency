...
async function seedStudentEmbeddings() {
  console.log('Starting seed process for all remaining barber student exam records...');
  ...
  while (hasMore) {
    ...
    for (const rec of rows) {
      try {
        ...
        const bio = `${rec.first_name} ${rec.last_name} took the TX Class A Barber ${rec.test_type} English exam at ${rec.school_name} on ${rec.test_date}, and ${rec.result === 'PASS' ? 'passed' : 'failed'} with a score of ${rec.score}%. ${attemptPhrase}`;
  ...
  console.log('Finished seeding ALL barber student exam record embeddings!');
}
