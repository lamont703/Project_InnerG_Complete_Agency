// Missouri twin of scripts/discover_and_stage_businesses.js (the
...
// (looks for "MO"/"Missouri" instead of "TX"/"Texas"), and one
...
// it here would silently orphan every Missouri finding from all four —
...
// Missouri data; they process it exactly like Texas data, since as far
...
// chain with both a Texas and a Missouri location) automatically once
// Missouri rows start publishing.
//
...
//
// Three modes:
//   node scripts/discover_and_stage_businesses_missouri.js "Kansas City MO"
//     Manual — discover exactly the city you name.
//   node scripts/discover_and_stage_businesses_missouri.js
...
//     MISSOURI CHANGE / real caveat: Google Ads Agent's directives don't
//     currently carry a state field, only a bare city name — if that agent
//     is ever run for Missouri too, an ambiguously-named city could in
//     theory get picked up by both this script's auto mode and the Texas
//     script's auto mode. Not a concern today (no Missouri keyword
//     research has been run), but worth a second look before relying on
//     auto mode here once it has.
//   node scripts/discover_and_stage_businesses_missouri.js --all-cities
//     State-wide sweep — every city in MO_CITIES below, one after another,
...
// MISSOURI CHANGE — the Texas script's TX_CITIES was an established,
...
// Missouri's ~34 largest cities by population, covering the Kansas City and St. Louis metros, the Springfield
// area, mid-Missouri, and the Ozarks — not
...
const MO_CITIES = [
  ...
  'lees summit', 'o fallon', 'st joseph', 'st charles', 'st peters',
...
async function extractFullDetail(page, name, city) {
  ...
  const detail = await page.evaluate(() => {
    ...
    const addressLine = lines.find((l) => /\d/.test(l) && /(MO|Missouri)\b|\b\d{5}\b/.test(l) && l.length < 90 && !/^\(/.test(l));
...
async function discoverCity(browser, cityArg, cityLabel, candidateMap, candidatePhoneMap) {
  ...
  for (const table of ACTIVE_TABLES) {
    ...
    // name Set would incorrectly skip a real Missouri candidate that
...
async function run() {
  ...
  let targets;
  if (ALL_CITIES_MODE) {
    targets = MO_CITIES.map((c) => ({ cityArg: `${titleCase(c)} MO`, cityLabel: titleCase(c), sourceDirective: null }));
    console.log(`State-wide sweep mode: ${targets.length} Missouri cities queued.`);
    ...
    targets = [{ cityArg, cityLabel: cityArg.replace(/\s*MO$/i, '').trim(), sourceDirective: null }];
  } else {
    ...
    if (approved.length === 0) {
      ...
      console.log('Either approve one at /admin/agent-directives, or run manually: node scripts/discover_and_stage_businesses_missouri.js "City MO"');
      return;
    }
    targets = approved.map((d) => ({ cityArg: `${d.evidence.city} MO`, cityLabel: d.evidence.city, sourceDirective: d }));
