...
async function seedCosmetologySchools() {
  console.log('Starting seed process for all remaining cosmetology schools...');
  ...
  while (hasMore) {
    ...
    for (const school of schools) {
      try {
        const bio = `${school.school_name} is a cosmetology school (${school.license_type || 'Cosmetology School'}) in ${school.city || school.formatted_address || 'Texas'}.
        ${school.accreditor_name ? `Accredited by ${school.accreditor_name}.` : 'State Licensed by TDLR.'}
  ...
  console.log('Finished seeding ALL cosmetology school embeddings!');
}

seedCosmetologySchools();
