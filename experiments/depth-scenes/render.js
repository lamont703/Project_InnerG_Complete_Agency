 ...
 *
 *   node experiments/depth-scenes/render.js
 *   node experiments/depth-scenes/render.js --fps=30 --strength=0.05
 *
...
(async () => {
  ...
  await page.goto(`file://${path.join(HERE, "scenes.html")}?w=${W}&h=${H}&strength=${STRENGTH}`, { waitUntil: "networkidle0" });
  ...
  const scenes = await page.evaluate(() => window.__SCENES.map((s) => ({ seconds: s.seconds, move: s.move })));
  const seconds = scenes.reduce((s, x) => s + x.seconds, 0);
  const total = Math.round(seconds * FPS);
  console.log(`  source ${dims.w}x${dims.h} -> ${W}x${H}`);
  scenes.forEach((s, i) => console.log(`    scene ${i + 1}: ${s.seconds}s ${s.move}`));
  ...
  const out = path.join(HERE, "three-scene.mp4");
  execFileSync(ffmpeg, [
