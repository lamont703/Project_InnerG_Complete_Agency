 ...
 *
 *   node experiments/depth-kinetic/render.js
 *   node experiments/depth-kinetic/render.js --fps=30 --strength=0.05
 *
...
(async () => {
  ...
  await page.goto(`file://${path.join(HERE, "kinetic.html")}?w=${W}&h=${H}&strength=${STRENGTH}`, { waitUntil: "networkidle0" });
  ...
  const beats = await page.evaluate(() => window.__BEATS.map((b) => b.to));
  const seconds = beats.length * 0.75;
  const total = Math.round(seconds * FPS);
  console.log(`  source ${dims.w}x${dims.h} -> ${W}x${H}`);
  console.log(`    ${beats.length} beats at 0.75s, transitions: ${beats.join(" -> ")}`);
  console.log(`  ${seconds}s, ${total} frames at ${FPS}fps`);

  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "kinetic-"));
  ...
  const out = path.join(HERE, "kinetic.mp4");
  execFileSync(ffmpeg, [
