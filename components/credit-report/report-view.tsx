...
import type { CreditReport, PaymentStatus } from "@/lib/credit-report/model";
import { BANDS } from "@/lib/credit-report/model";

/**
 * The report itself, drawn once.
 *
 * LIFTED OUT OF app/admin/credit-report/page.tsx rather than copied. Three
 * surfaces render this — the admin prototype, a member looking at their own
 * record, and whoever they hand a share link to — and a payment history that
 * renders differently depending on who is looking is the exact thing this
 * product cannot afford. The score a barber sees has to be the score the shop
 * sees, character for character.
 *
 * PURE PRESENTATION. It takes a built CreditReport and a subject; it does no
 * scoring, no fetching and no authorisation. Deciding whether the viewer is
 * allowed to see this belongs to the page, and a component that could be
 * mistaken for the gate would eventually be trusted as one.
 */
 ...
 *
 * Built from whatever is actually known rather than a fixed template. The
 * prototype could hardcode a handle, a licence type and a member-since date
 * because it invented all three; a real member may have none of them, and
 * "· undefined ()" under somebody's name reads as a broken record.
 */
function subjectLine(s: ReportSubject): string {
  const licence = s.licenceType
    ? s.licenceState
      ? `${s.licenceType} (${s.licenceState})`
      : s.licenceType
    : null;
  return [s.handle, licence, s.memberSince ? `Member since ${s.memberSince}` : null]
    .filter(Boolean)
    .join(" · ");
}
...
export function CreditReportView({
  ...
  const subtitle = subjectLine(subject);
        ...
        <header className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
          <div className="flex flex-wrap items-start justify-between gap-6">
            <div>
              ...
              <h1 className="mt-1 text-2xl font-black text-slate-900">{subject.name}</h1>
