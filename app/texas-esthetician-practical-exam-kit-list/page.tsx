...
export const metadata = {
  ...
  alternates: { canonical: `${SITE_URL}/texas-esthetician-practical-exam-kit-list` },
};
...
const { groups: KIT_GROUPS, providedOnSite: PROVIDED_ON_SITE, sections: SECTIONS, rules: RULES } = TEXAS_ESTHETICIAN_KIT;
  ...
  return (
    <div className="min-h-screen light bg-white text-slate-950">
      ...
      <main className="max-w-4xl mx-auto px-4 sm:px-6 pt-28 pb-10 sm:pb-14">
        ...
        <AgentInvite
          ...
          questions={[
            ...
            "How many people took the Texas esthetician exam and how many passed?",

            "Which schools in my city have the best esthetician outcomes?",

            "What do esthetician chairs rent for around me?",
        ...
        <div className="no-print flex flex-wrap gap-3 mb-16">
          ...
          <Link
            href="/texas-esthetician-exam-prep"
            className="inline-flex items-center gap-2 px-5 py-3 rounded-xl bg-white border border-slate-200 hover:bg-slate-50 text-slate-900 font-extrabold text-sm uppercase tracking-wider transition-colors"
          >
      ...
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(graph(
            {
            "@type": "FAQPage",
            "@id": `${SITE_URL}/texas-esthetician-practical-exam-kit-list#faqpage`,
      ...
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(graph(
            {
            "@type": "HowTo",
            "@id": `${SITE_URL}/texas-esthetician-practical-exam-kit-list#howto`,
            "isPartOf": ref(WEBSITE_ID),
            "publisher": ref(ORG_ID), author: authorSchema(), name: "Texas Esthetician Practical Exam — Station Order", description: "The 8 timed sections of the Texas Esthetician practical exam in order, per the PSI/TDLR Candidate Information Bulletin effective January 1, 2026.", totalTime: "PT1H41M", step: SECTIONS.map((s, i) => ({ "@type": "HowToStep", position: i + 1, name: s.name, text: s.notes.join(" ") })) },
          )) }} />
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(graph(
            {
            "@type": "ItemList",
            "@id": `${SITE_URL}/texas-esthetician-practical-exam-kit-list#itemlist`, name: "Texas Esthetician Practical Exam Kit List (2026)", itemListElement: KIT_GROUPS.flatMap((g) => g.items).map((item, i) => ({ "@type": "ListItem", position: i + 1, name: item.label })) },
          )) }} />
    </div>
  );
}
