...
const FAQS = [
  {
    q: "How many barbershops and salons are in Katy, TX?",
    a: "Real, currently-tracked count: the number shown at the top of this page, verified directly against real business addresses in Katy.",
  },
  {
    q: "Is this list the same as a generic directory?",
    a: "No — every listing here is a real business with a real, verified Katy address, ranked by real customer ratings and review counts, not a paid placement or a scraped list.",
  },
];

export default function KatyBarbershopsSalonsPage() {
  const [listings, setListings] = useState<KatyListing[]>([]);
  const [filter, setFilter] = useState<"All" | "Barbershop" | "Salon">("All");
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    fetchKatyListings().then((result) => {
      setListings(result);
      setLoading(false);
    });
  }, []);
