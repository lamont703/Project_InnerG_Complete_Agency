...
export default function HistoricalPerformancePage() {
    ...
    // 1. Initial Load: Get list of available schools
    useEffect(() => {
    ...
    // 2. Fetch school timeline and predictions when selection changes
    useEffect(() => {
        ...
        async function fetchSchoolData() {
            ...
            try {
                const res = await fetch(`/api/texas-historical-performance?schoolCode=${selectedCode}`);
                const data = await res.json();
    ...
    // 3. SVG Custom Chart Math for 10 Months (4 actual + 6 predictions)
    const chartConfig = useMemo(() => {
        ...
        // Map months (10 items) to X coordinates
        ...
        // Separate points into Historical (0-3) and Prediction (3-9)
        const histLength = 4; // Jan, Feb, Mar, Apr
        ...
        // School prediction coordinates (starts from April bridge point at index 3)
    ...
    // Calculate predictions indicators (October is the last point, at index 9)
    const octoberProjection = useMemo(() => {
        if (timeline.length < 10) return null;
        return timeline[9].schoolPassRate;
    }, [timeline]);

    const isBelowNaccas = overallPassRate < 70;
    const isProjectedBelowNaccas = octoberProjection !== null && octoberProjection < 70;
                ...
                <header className="hero-section">
                    <div className="flex justify-center mb-6">
                        <div className="bg-violet-500/10 text-violet-600 px-4 py-1.5 rounded-full text-sm font-semibold border border-violet-500/20 flex items-center gap-2">
                            <Sparkles size={16} />
                            6-Month Predictive AI Analytics
                        </div>
                    </div>
                    <h1 className="hero-title">Texas Barber School <br />Historical & Predictive Performance</h1>
                    <p className="hero-subtitle">
                        Analyze historical written board pass-rates alongside a 6-month AI linear regression forecast.
                    </p>
                </header>
                    ...
                    <>
                        ...
                        <div className="metrics-grid">
                            <div className="metric-card">
                                <span className="metric-label">Overall 2026 Pass Rate</span>
                            ...
                            <div className="metric-card metric-card-ai">
                                <span className="metric-label flex items-center gap-1">
                                    <Sparkles size={12} className="text-violet-500" /> Oct 2026 Forecast
                                </span>
                                <span className={`metric-value ${isProjectedBelowNaccas ? 'status-text-danger' : 'status-text-success'}`}>
                                    {octoberProjection !== null ? `${octoberProjection}%` : "N/A"}
                                </span>
                            </div>
                        </div>
                        ...
                        <div className="chart-card">
                            <div className="chart-title-wrapper">
                                <div className="chart-title flex items-center gap-2">
                                    ...
                                    Written Pass Rates & 6-Month AI Projections
                                </div>
                                <div className="chart-legend">
                                    ...
                                    <div className="legend-item">
                                        <div className="legend-color bg-blue-600" style={{ opacity: 0.5, border: '1px dashed #2563eb' }} />
                                        <span className="text-violet-600">AI 6-Mo Forecast</span>
                                    </div>
                                    <div className="legend-item">
                            ...
                            <div className="callout-content">
                                ...
                                <p className="callout-desc">
                                    {isProjectedBelowNaccas ? (
                                        <span>
                                            <strong>Accreditation Risk Warning:</strong> 6-month mathematical trend models project your written exam success rate to slide to <strong>{octoberProjection}%</strong> by October 2026. This falls below the mandatory <strong>70% NACCAS standard</strong>. Immediate action is required to avoid threat to financial aid.
                                        </span>
                                    ) : (
                                        <span>
                                            <strong>Positive Outlook:</strong> 6-month performance metrics project your written board success rate to stay highly compliant at <strong>{octoberProjection}%</strong> through October 2026. Keep leveraging active diagnostic simulators to protect your accreditation safety margin!
                                        </span>
                                    )}
                                </p>
                            </div>
