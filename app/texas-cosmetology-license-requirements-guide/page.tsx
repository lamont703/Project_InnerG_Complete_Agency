...
const REQ = findRequirement('cosmetologist')!;
const SOURCE = TDLR_SOURCES.find((s) => s.id === 'apply-cosmetologist');

export const metadata = {
  title: 'Texas Cosmetology Operator License Requirements (2026)',
  description: "What a Texas cosmetology operator licence requires — 1,000 hours, $50 fee, 2-year term — and the exact TDLR process, sourced from the Department's own application pages.",
  keywords: ["texas cosmetology operator license requirements", "how to get a cosmetology operator license in texas", "texas cosmetology operator license", "cosmetology operator license texas cost", "texas cosmetology operator license application", "tdlr license requirements texas", "cosmetology operator hours texas"],
  openGraph: { title: 'Texas Cosmetology Operator License Requirements (2026)', description: "What a Texas cosmetology operator licence requires — 1,000 hours, $50 fee, 2-year term — and the exact TDLR process, sourced from the Department's own application pages." },
  alternates: { canonical: `${SITE_URL}/texas-cosmetology-license-requirements-guide` },
};

const FAQS = [
  {
    q: "How many hours do you need for a Texas cosmetology operator licence?",
    a: "1,000 hours of instruction at a school licensed in Texas."
  },
  {
    q: "How much does a Texas cosmetology operator licence cost?",
    a: "$50 to apply, non-refundable, and the licence is valid two years from the date of issue."
  },
  {
    q: "How long is a Texas cosmetology operator licence valid?",
    a: "Two years from the date of issue, then it renews on a two-year cycle."
  },
  {
    q: "Which exams do you have to pass?",
    a: "Both the written and the practical, administered by PSI on TDLR's behalf. Written eligibility generates at 900 hours, so you can sit it before the course ends; the practical comes only after all hours are complete and the written is passed."
  }
];

export default function TexasCosmetologyOperatorRequirementsPage() {
  return (
    <div className="min-h-screen light bg-white text-slate-950">
      ...
      <main className="mx-auto max-w-4xl px-4 sm:px-6 pt-28 pb-16">
        ...
        <p className="mb-10 max-w-2xl text-base leading-relaxed text-slate-600 sm:text-lg">
          What it takes to be licensed as a cosmetology operator in Texas — the hours, the fee, the exams and the order they happen in.
        </p>
        ...
        <AgentInvite questions={questionsForSlug("texas-cosmetology-license-requirements-guide")!} />
      ...
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(graph(
            {
            "@type": "FAQPage",
            "@id": `${SITE_URL}/texas-cosmetology-license-requirements-guide#faqpage`,
      ...
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(graph(
            {
            "@type": "HowTo",
            "@id": `${SITE_URL}/texas-cosmetology-license-requirements-guide#howto`,
            "isPartOf": ref(WEBSITE_ID),
            "publisher": ref(ORG_ID), author: authorSchema(), name: 'Texas Cosmetology Operator License Requirements (2026)', description: "What a Texas cosmetology operator licence requires — 1,000 hours, $50 fee, 2-year term — and the exact TDLR process, sourced from the Department's own application pages.", estimatedCost: { "@type": "MonetaryAmount", currency: "USD", value: REQ.feeUsd }, step: REQ.conditions.map((c, i) => ({ "@type": "HowToStep", position: i + 1, text: c })) },
          )) }} />
    </div>
  );
}
