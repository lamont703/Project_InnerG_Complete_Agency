...
const REQ = findRequirement('cosmetology-school')!;
const SOURCE = TDLR_SOURCES.find((s) => s.id === 'schools-apply');

export const metadata = {
  title: 'Texas Cosmetology School License Requirements (2026)',
  description: "What a Texas cosmetology school licence requires — $580 fee, 2-year term — and the exact TDLR process, sourced from the Department's own application pages.",
  keywords: ["texas cosmetology school license requirements", "how to get a cosmetology school license in texas", "texas cosmetology school license", "cosmetology school license texas cost", "texas cosmetology school license application", "tdlr license requirements texas"],
  openGraph: { title: 'Texas Cosmetology School License Requirements (2026)', description: "What a Texas cosmetology school licence requires — $580 fee, 2-year term — and the exact TDLR process, sourced from the Department's own application pages." },
  alternates: { canonical: `${SITE_URL}/texas-cosmetology-school-license-requirements-guide` },
};

const FAQS = [
  {
    q: "How many hours do you need for a Texas cosmetology school licence?",
    a: "There is no course-hour requirement \u2014 a cosmetology school licence is a business licence, not a training credential."
  },
  {
    q: "How much does a Texas cosmetology school licence cost?",
  ...
  {
    q: "How long is a Texas cosmetology school licence valid?",
    a: "Two years from the date of issue, then it renews on a two-year cycle."
  }
];

export default function TexasCosmetologySchoolRequirementsPage() {
  return (
    <div className="min-h-screen light bg-white text-slate-950">
      ...
      <main className="mx-auto max-w-4xl px-4 sm:px-6 pt-28 pb-16">
        ...
        <h1 className="mb-3 text-3xl font-black leading-tight tracking-tight text-slate-950 sm:text-4xl">
          Texas Cosmetology School License Requirements
        </h1>
        <p className="mb-10 max-w-2xl text-base leading-relaxed text-slate-600 sm:text-lg">
          What it takes to license a cosmetology school licence in Texas — the fee, the documents, and what private schools must file that public schools do not.
        </p>
      ...
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(graph(
            {
            "@type": "FAQPage",
            "@id": `${SITE_URL}/texas-cosmetology-school-license-requirements-guide#faqpage`,
      ...
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(graph(
            {
            "@type": "HowTo",
            "@id": `${SITE_URL}/texas-cosmetology-school-license-requirements-guide#howto`,
            "isPartOf": ref(WEBSITE_ID),
            "publisher": ref(ORG_ID), author: authorSchema(), name: 'Texas Cosmetology School License Requirements (2026)', description: "What a Texas cosmetology school licence requires — $580 fee, 2-year term — and the exact TDLR process, sourced from the Department's own application pages.", estimatedCost: { "@type": "MonetaryAmount", currency: "USD", value: REQ.feeUsd }, step: REQ.conditions.map((c, i) => ({ "@type": "HowToStep", position: i + 1, text: c })) },
          )) }} />
    </div>
  );
}
