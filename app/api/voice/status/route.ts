...
export async function POST(req: NextRequest) {
  ...
  // DELIBERATELY DOES NOT TOUCH `answered` OR `billable`. This fires when the
  // whole call ends, which is AFTER /completed has decided them — writing here
  // would clobber a correct verdict with one derived from the inbound leg,
  // whose duration includes the agent prompt and the ringing.
  //
  // A row that exists only because of this handler is a caller who hung up
  // during the prompt and never reached a school. Recording those is the point:
  // it is the only way to know how many people the agent loses before it
  // connects anyone.
  try {
