...
const REQ = findRequirement('barber-establishment')!;
const SOURCE = TDLR_SOURCES.find((s) => s.id === 'establishments-apply');

export const metadata = {
  title: 'Texas Barber Establishment License Requirements (2026)',
  description: "What a Texas barber establishment licence requires — $78 fee, 2-year term — and the exact TDLR process, sourced from the Department's own application pages.",
  keywords: ["texas barber establishment license requirements", "how to get a barber establishment license in texas", "texas barber establishment license", "barber establishment license texas cost", "texas barber establishment license application", "tdlr license requirements texas"],
  openGraph: { title: 'Texas Barber Establishment License Requirements (2026)', description: "What a Texas barber establishment licence requires — $78 fee, 2-year term — and the exact TDLR process, sourced from the Department's own application pages." },
  alternates: { canonical: `${SITE_URL}/texas-barber-establishment-license-requirements-guide` },
};

const FAQS = [
  {
    q: "How many hours do you need for a Texas barber establishment licence?",
    a: "There is no course-hour requirement \u2014 a barbershop establishment licence is a business licence, not a training credential."
  },
  {
    q: "How much does a Texas barber establishment licence cost?",
    a: "$78 to apply, non-refundable, and the licence is valid two years from the date of issue."
  },
  {
    q: "How long is a Texas barber establishment licence valid?",
...
export default function TexasBarberEstablishmentRequirementsPage() {
  return (
    <div className="min-h-screen light bg-white text-slate-950">
      ...
      <main className="mx-auto max-w-4xl px-4 sm:px-6 pt-28 pb-16">
        ...
        <h1 className="mb-3 text-3xl font-black leading-tight tracking-tight text-slate-950 sm:text-4xl">
          Texas Barber Establishment License Requirements
        </h1>
        <p className="mb-10 max-w-2xl text-base leading-relaxed text-slate-600 sm:text-lg">
          What TDLR requires to license a barbershop establishment licence in Texas — the fee, the conditions, and the deadline that voids applications.
        </p>
      ...
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(graph(
            {
            "@type": "FAQPage",
            "@id": `${SITE_URL}/texas-barber-establishment-license-requirements-guide#faqpage`,
      ...
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(graph(
            {
            "@type": "HowTo",
            "@id": `${SITE_URL}/texas-barber-establishment-license-requirements-guide#howto`,
            "isPartOf": ref(WEBSITE_ID),
            "publisher": ref(ORG_ID), author: authorSchema(), name: 'Texas Barber Establishment License Requirements (2026)', description: "What a Texas barber establishment licence requires — $78 fee, 2-year term — and the exact TDLR process, sourced from the Department's own application pages.", estimatedCost: { "@type": "MonetaryAmount", currency: "USD", value: REQ.feeUsd }, step: REQ.conditions.map((c, i) => ({ "@type": "HowToStep", position: i + 1, text: c })) },
          )) }} />
    </div>
  );
}
