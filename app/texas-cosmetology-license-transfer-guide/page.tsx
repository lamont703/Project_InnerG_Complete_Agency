...
const REQ = findRequirement('cosmetology-transfer')!;
const SOURCE = TDLR_SOURCES.find((s) => s.id === 'barber-to-cosmetologist');

export const metadata = {
  title: 'Texas Barber to Cosmetology Operator Crossover Guide (2026)',
...
const FAQS = [
  {
    q: "How many hours do you need for a Texas barber to cosmetology operator licence?",
    a: "300 hours of instruction at a school licensed in Texas."
  },
  {
    q: "How much does a Texas barber to cosmetology operator licence cost?",
    a: "$50 to apply, non-refundable, and the licence is valid two years from the date of issue."
  },
  {
    q: "How long is a Texas barber to cosmetology operator licence valid?",
...
export default function TexasBarberToCosmetologyOperatorRequirementsPage() {
  return (
    <div className="min-h-screen light bg-white text-slate-950">
      ...
      <main className="mx-auto max-w-4xl px-4 sm:px-6 pt-28 pb-16">
        ...
        <h1 className="mb-3 text-3xl font-black leading-tight tracking-tight text-slate-950 sm:text-4xl">
          Texas Barber to Cosmetology Operator License Requirements
        </h1>
        ...
        <AgentInvite questions={questionsForSlug("texas-cosmetology-license-transfer-guide")!} />
      ...
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(graph(
            {
            "@type": "FAQPage",
            "@id": `${SITE_URL}/texas-cosmetology-license-transfer-guide#faqpage`,
      ...
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(graph(
            {
            "@type": "HowTo",
            "@id": `${SITE_URL}/texas-cosmetology-license-transfer-guide#howto`,
            "isPartOf": ref(WEBSITE_ID),
            "publisher": ref(ORG_ID), author: authorSchema(), name: 'Texas Barber to Cosmetology Operator Crossover Guide (2026)', description: "What a Texas barber to cosmetology operator licence requires — 300 hours, $50 fee, 2-year term — and the exact TDLR process, sourced from the Department's own application pages.", estimatedCost: { "@type": "MonetaryAmount", currency: "USD", value: REQ.feeUsd }, step: REQ.conditions.map((c, i) => ({ "@type": "HowToStep", position: i + 1, text: c })) },
          )) }} />
    </div>
  );
}
