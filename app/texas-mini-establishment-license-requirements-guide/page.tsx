...
const REQ = findRequirement('mini-establishment')!;
const SOURCE = TDLR_SOURCES.find((s) => s.id === 'mini-establishment');

export const metadata = {
  title: 'Texas Mini-Establishment License Requirements (2026)',
  description: "What a Texas mini-establishment licence requires — $70 fee, 2-year term — and the exact TDLR process, sourced from the Department's own application pages.",
  keywords: ["texas mini-establishment license requirements", "how to get a mini-establishment license in texas", "texas mini-establishment license", "mini-establishment license texas cost", "texas mini-establishment license application", "tdlr license requirements texas"],
  openGraph: { title: 'Texas Mini-Establishment License Requirements (2026)', description: "What a Texas mini-establishment licence requires — $70 fee, 2-year term — and the exact TDLR process, sourced from the Department's own application pages." },
  alternates: { canonical: `${SITE_URL}/texas-mini-establishment-license-requirements-guide` },
};

const FAQS = [
  {
    q: "How many hours do you need for a Texas mini-establishment licence?",
    a: "There is no course-hour requirement \u2014 a mini-establishment licence is a business licence, not a training credential."
  },
  {
    q: "How much does a Texas mini-establishment licence cost?",
    a: "$70 to apply, non-refundable, and the licence is valid two years from the date of issue."
  },
  {
    q: "How long is a Texas mini-establishment licence valid?",
...
export default function TexasMiniEstablishmentRequirementsPage() {
  return (
    <div className="min-h-screen light bg-white text-slate-950">
      ...
      <main className="mx-auto max-w-4xl px-4 sm:px-6 pt-28 pb-16">
        ...
        <h1 className="mb-3 text-3xl font-black leading-tight tracking-tight text-slate-950 sm:text-4xl">
          Texas Mini-Establishment License Requirements
        </h1>
        <p className="mb-10 max-w-2xl text-base leading-relaxed text-slate-600 sm:text-lg">
          What TDLR requires to license a mini-establishment licence in Texas — the fee, the conditions, and the deadline that voids applications.
        </p>
      ...
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(graph(
            {
            "@type": "FAQPage",
            "@id": `${SITE_URL}/texas-mini-establishment-license-requirements-guide#faqpage`,
      ...
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(graph(
            {
            "@type": "HowTo",
            "@id": `${SITE_URL}/texas-mini-establishment-license-requirements-guide#howto`,
            "isPartOf": ref(WEBSITE_ID),
            "publisher": ref(ORG_ID), author: authorSchema(), name: 'Texas Mini-Establishment License Requirements (2026)', description: "What a Texas mini-establishment licence requires — $70 fee, 2-year term — and the exact TDLR process, sourced from the Department's own application pages.", estimatedCost: { "@type": "MonetaryAmount", currency: "USD", value: REQ.feeUsd }, step: REQ.conditions.map((c, i) => ({ "@type": "HowToStep", position: i + 1, text: c })) },
          )) }} />
    </div>
  );
}
