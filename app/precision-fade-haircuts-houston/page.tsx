...
import { Star, Loader2, Scissors, DollarSign, MapPin, ArrowRight } from "lucide-react";
import { fetchFadeBarbers, type FadeBarberListing } from "./actions";
...
const FAQS_STATIC = [
  {
    q: "What is a precision fade?",
    a: "A precision fade is a haircut where the hair gradually shortens from longer at the top to skin-short at the sides and back, cut with clippers to create a clean, blended gradient rather than a single guard-length all over.",
  },
  {
    q: "How do I find a barber who specializes in fades near me?",
    a: "Look for a barber whose own service menu explicitly lists a fade (not just \"haircut\") — that's a stronger signal than a shop's general description. The listings on this page are pulled from real, named services each barber currently lists.",
  },
];

export default function PrecisionFadeHoustonPage() {
  const [listings, setListings] = useState<FadeBarberListing[]>([]);
  const [avgFadePrice, setAvgFadePrice] = useState<number | null>(null);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    fetchFadeBarbers().then((result) => {
      setListings(result.listings);
      setAvgFadePrice(result.avgFadePrice);
      setLoading(false);
    });
  }, []);

  const priceFaq = {
    q: "How much does a fade cost in Houston?",
    a: avgFadePrice
      ? `Based on real, currently-listed prices from Houston barbers who specialize in fades, the average price is around $${avgFadePrice}. Individual prices vary by barber and exact style — see the real listings on this page for current pricing.`
      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6 pt-28 pb-10 sm:pb-16">
        ...
        <div className="text-center mb-10">
          ...
          <h1 className="text-3xl sm:text-4xl font-black tracking-tight text-slate-950 leading-tight mb-3">
            Precision Fade Haircuts in Houston
          </h1>
          <p className="text-slate-600 text-base sm:text-lg leading-relaxed max-w-xl mx-auto">
            Real Houston barbers who list a fade as a named service on their own menu — ranked by real customer
            ratings, with real prices. Not a generic directory of every barbershop in the city.
          </p>
        </div>
        ...
        {loading ? (
          ...
          <div className="bg-amber-50 border border-amber-200 rounded-2xl p-6 mb-10">
            <p className="text-sm text-amber-900 leading-relaxed">
              No fade-specialist listings found right now — check back soon, this list updates as barbers update
              their service menus.
            </p>
          </div>
        ) : (
          <div className="space-y-4 mb-16">
            <p className="text-sm font-bold text-slate-700">
              {listings.length} Houston barber{listings.length === 1 ? "" : "s"} who list a fade as a real service:
            </p>
                  ...
                  <div className="flex flex-wrap items-center gap-x-3 gap-y-1 mt-1.5">
                    {barber.fadePrice != null && (
                      <span className="inline-flex items-center gap-1 text-xs font-black text-emerald-700 bg-emerald-50 border border-emerald-200 rounded-full px-2 py-0.5">
                        <DollarSign className="w-3 h-3" /> ${barber.fadePrice} — {barber.fadeServiceName}
                      </span>
                    )}
                    {barber.rating != null && (
      ...
      <script
        ...
        dangerouslySetInnerHTML={{
          __html: JSON.stringify(graph(
            {
            "@type": "FAQPage",
            "@id": `${SITE_URL}/precision-fade-haircuts-houston#faqpage`,
