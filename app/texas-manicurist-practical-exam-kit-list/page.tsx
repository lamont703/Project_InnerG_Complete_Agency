...
export const metadata = {
  title: "Texas Manicurist (Nail Tech) State Board Practical Exam Kit Checklist (2026)",
  description:
  ...
  alternates: { canonical: `${SITE_URL}/texas-manicurist-practical-exam-kit-list` },
};
...
const { groups: KIT_GROUPS, providedOnSite: PROVIDED_ON_SITE, sections: SECTIONS, rules: RULES } = TEXAS_MANICURIST_KIT;
  ...
  return (
    <div className="min-h-screen light bg-white text-slate-950">
      ...
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(graph(
            {
            "@type": "FAQPage",
            "@id": `${SITE_URL}/texas-manicurist-practical-exam-kit-list#faqpage`,
      ...
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(graph(
            {
            "@type": "HowTo",
            "@id": `${SITE_URL}/texas-manicurist-practical-exam-kit-list#howto`,
            "isPartOf": ref(WEBSITE_ID),
            "publisher": ref(ORG_ID), author: authorSchema(), name: "Texas Manicurist Practical Exam — Station Order", description: "The 6 timed sections of the Texas Manicurist practical exam in order, per the PSI/TDLR Candidate Information Bulletin effective January 1, 2026.", totalTime: "PT1H21M", step: SECTIONS.map((s, i) => ({ "@type": "HowToStep", position: i + 1, name: s.name, text: s.notes.join(" ") })) },
          )) }} />
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(graph(
            {
            "@type": "ItemList",
            "@id": `${SITE_URL}/texas-manicurist-practical-exam-kit-list#itemlist`, name: "Texas Manicurist Practical Exam Kit List (2026)", itemListElement: KIT_GROUPS.flatMap((g) => g.items).map((item, i) => ({ "@type": "ListItem", position: i + 1, name: item.label })) },
          )) }} />
    </div>
  );
}
