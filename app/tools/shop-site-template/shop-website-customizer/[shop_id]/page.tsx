...
import ShopPreviewClient from "./client"
...
export default async function ShopPreviewPage({
  ...
  try {
    ...
    if (shop) {
      if (shop.site_config) {
        // If they already have a customized config saved, use it
        initialConfig = shop.site_config as SiteConfig
      } else {
        // Otherwise build a dynamic config from their row data
        initialConfig = {
          ...defaultSiteConfig,
          hero: {
    ...
    console.error("Error fetching shop data for customizer preview:", err)
  }

  return <ShopPreviewClient initialConfig={initialConfig} shopId={shop_id} />
}
