...
const REQ = findRequirement('barber')!;
const SOURCE = TDLR_SOURCES.find((s) => s.id === 'apply-barber');

export const metadata = {
  title: 'Texas Class A Barber License Requirements (2026)',
  description: "What a Texas class a barber licence requires — 1,000 hours, $50 fee, 2-year term — and the exact TDLR process, sourced from the Department's own application pages.",
  keywords: ["texas class a barber license requirements", "how to get a class a barber license in texas", "texas class a barber license", "class a barber license texas cost", "texas class a barber license application", "tdlr license requirements texas", "class a barber hours texas"],
  openGraph: { title: 'Texas Class A Barber License Requirements (2026)', description: "What a Texas class a barber licence requires — 1,000 hours, $50 fee, 2-year term — and the exact TDLR process, sourced from the Department's own application pages." },
  alternates: { canonical: `${SITE_URL}/texas-barber-license-requirements-guide` },
};

const FAQS = [
  {
    q: "How many hours do you need for a Texas class a barber licence?",
    a: "1,000 hours of instruction at a school licensed in Texas."
  },
  {
    q: "How much does a Texas class a barber licence cost?",
    a: "$50 to apply, non-refundable, and the licence is valid two years from the date of issue."
  },
  {
    q: "How long is a Texas class a barber licence valid?",
...
export default function TexasClassABarberRequirementsPage() {
  return (
    <div className="min-h-screen light bg-white text-slate-950">
      ...
      <main className="mx-auto max-w-4xl px-4 sm:px-6 pt-28 pb-16">
        ...
        <h1 className="mb-3 text-3xl font-black leading-tight tracking-tight text-slate-950 sm:text-4xl">
          Texas Class A Barber License Requirements
        </h1>
        <p className="mb-10 max-w-2xl text-base leading-relaxed text-slate-600 sm:text-lg">
          What it takes to be licensed as a class a barber in Texas — the hours, the fee, the exams and the order they happen in.
        </p>
        ...
        <AgentInvite questions={questionsForSlug("texas-barber-license-requirements-guide")!} />
      ...
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(graph(
            {
            "@type": "FAQPage",
            "@id": `${SITE_URL}/texas-barber-license-requirements-guide#faqpage`,
      ...
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(graph(
            {
            "@type": "HowTo",
            "@id": `${SITE_URL}/texas-barber-license-requirements-guide#howto`,
            "isPartOf": ref(WEBSITE_ID),
            "publisher": ref(ORG_ID), author: authorSchema(), name: 'Texas Class A Barber License Requirements (2026)', description: "What a Texas class a barber licence requires — 1,000 hours, $50 fee, 2-year term — and the exact TDLR process, sourced from the Department's own application pages.", estimatedCost: { "@type": "MonetaryAmount", currency: "USD", value: REQ.feeUsd }, step: REQ.conditions.map((c, i) => ({ "@type": "HowToStep", position: i + 1, text: c })) },
          )) }} />
    </div>
  );
}
