...
export default async function ShopDayMapPage() {
  ...
  const [
    ...
    fetchAllRows(supabase, "agent_barber_school_leads",
      "id, slug, latitude, longitude, school_name, city, accreditation_status, formatted_address",
      (q) => q.not("latitude", "is", null).not("longitude", "is", null)),
    fetchAllRows(supabase, "agent_barber_leads",
      "id, slug, latitude, longitude, name, address, desired_pay_structure, status",
      (q) => q.not("latitude", "is", null).not("longitude", "is", null)),
    fetchAllRows(supabase, "agent_cosmetology_school_leads",
      "id, slug, latitude, longitude, school_name, city, accreditation_status, formatted_address",
      (q) => q.not("latitude", "is", null).not("longitude", "is", null)),
    fetchAllRows(supabase, "agent_cosmetologist_leads",
      "id, slug, latitude, longitude, name, address, desired_pay_structure",
      (q) => q.not("latitude", "is", null).not("longitude", "is", null)),
    fetchAllRows(supabase, "agent_salon_leads",
      "id, slug, latitude, longitude, shop_name, city, rent_type, booth_count_available, hiring_need, formatted_address, phone, email, rating, total_reviews",
      (q) => q.not("latitude", "is", null).not("longitude", "is", null)),
    fetchAllRows(supabase, "agent_barber_supply_store_leads",
      "id, slug, latitude, longitude, name, city, formatted_address, phone, website, rating, total_reviews",
      (q) => q.not("latitude", "is", null).not("longitude", "is", null)),
