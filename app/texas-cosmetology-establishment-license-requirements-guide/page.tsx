...
const REQ = findRequirement('cosmetology-establishment')!;
const SOURCE = TDLR_SOURCES.find((s) => s.id === 'establishments-apply');

export const metadata = {
  title: 'Texas Cosmetology Establishment License Requirements (2026)',
  description: "What a Texas cosmetology establishment licence requires — $78 fee, 2-year term — and the exact TDLR process, sourced from the Department's own application pages.",
  keywords: ["texas cosmetology establishment license requirements", "how to get a cosmetology establishment license in texas", "texas cosmetology establishment license", "cosmetology establishment license texas cost", "texas cosmetology establishment license application", "tdlr license requirements texas"],
  openGraph: { title: 'Texas Cosmetology Establishment License Requirements (2026)', description: "What a Texas cosmetology establishment licence requires — $78 fee, 2-year term — and the exact TDLR process, sourced from the Department's own application pages." },
  alternates: { canonical: `${SITE_URL}/texas-cosmetology-establishment-license-requirements-guide` },
};

const FAQS = [
  {
    q: "How many hours do you need for a Texas cosmetology establishment licence?",
    a: "There is no course-hour requirement \u2014 a salon establishment licence is a business licence, not a training credential."
  },
  {
    q: "How much does a Texas cosmetology establishment licence cost?",
    a: "$78 to apply, non-refundable, and the licence is valid two years from the date of issue."
  },
  {
    q: "How long is a Texas cosmetology establishment licence valid?",
    a: "Two years from the date of issue, then it renews on a two-year cycle."
  },
  {
    q: "How long do you have to finish the application?",
    a: "One year from the date TDLR receives it. Miss that and the application is void and you start again."
  }
];

export default function TexasCosmetologyEstablishmentRequirementsPage() {
  return (
    <div className="min-h-screen light bg-white text-slate-950">
      ...
      <main className="mx-auto max-w-4xl px-4 sm:px-6 pt-28 pb-16">
        ...
        <h1 className="mb-3 text-3xl font-black leading-tight tracking-tight text-slate-950 sm:text-4xl">
          Texas Cosmetology Establishment License Requirements
        </h1>
        <p className="mb-10 max-w-2xl text-base leading-relaxed text-slate-600 sm:text-lg">
          What TDLR requires to license a salon establishment licence in Texas — the fee, the conditions, and the deadline that voids applications.
        </p>
        ...
        <div className="mb-12">
          ...
          <div className="flex flex-wrap gap-3">
            ...
            <Link href="/texas-barber-school-license-requirements-guide" className="inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-xs font-extrabold uppercase tracking-wider text-slate-900 transition-colors hover:bg-slate-50">Barber School</Link>
      ...
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(graph(
            {
            "@type": "FAQPage",
            "@id": `${SITE_URL}/texas-cosmetology-establishment-license-requirements-guide#faqpage`,
      ...
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(graph(
            {
            "@type": "HowTo",
            "@id": `${SITE_URL}/texas-cosmetology-establishment-license-requirements-guide#howto`,
            "isPartOf": ref(WEBSITE_ID),
            "publisher": ref(ORG_ID), author: authorSchema(), name: 'Texas Cosmetology Establishment License Requirements (2026)', description: "What a Texas cosmetology establishment licence requires — $78 fee, 2-year term — and the exact TDLR process, sourced from the Department's own application pages.", estimatedCost: { "@type": "MonetaryAmount", currency: "USD", value: REQ.feeUsd }, step: REQ.conditions.map((c, i) => ({ "@type": "HowToStep", position: i + 1, text: c })) },
          )) }} />
    </div>
  );
}
