...
const EXAM = SPECIALTY_EXAMS.esthetician;
...
export const metadata = {
  ...
  keywords: ["texas esthetician exam", "texas esthetician practical exam", "esthetician state board exam texas", "texas esthetician written exam", "psi esthetician exam texas", "esthetician exam prep texas", "texas esthetician license exam", "esthetician practical exam criteria"],
  openGraph: {
    title: 'Texas Esthetician Exam Prep (2026): Practical Drill & Kit',
    description: 'Every scored criterion on the Texas esthetician practical exam, the 8 timed sections, and the written exam format — from the Jan 1, 2026 PSI/TDLR bulletin.',
  },
...
const FAQS = [
  {
    q: "How long is the Texas esthetician practical exam and what score do I need?",
    a: "The practical runs 1h 41m across 8 timed sections and is worth 76 points. You need 70% — 54 of 76 — to pass. Every criterion is worth one point and they are marked in a fixed order.",
  },
  {
    q: "What is on the Texas esthetician written exam?",
    a: "75 scored items in 105 minutes, with a 70% passing score. You must pass the written exam before practical eligibility can generate.",
  },
  {
    q: "What order are the esthetician practical sections performed in?",
    a: "Pre-Examination Set Up and Disinfection, then Cleansing, then Steaming, then Massage, then Mask and Moisturizing, then Waxing with Soft Wax, then Blood Exposure Incident, then End of Examination Disinfection. Steps performed out of order, or not completed within the time allowed, are not scored.",
  },
  {
    q: "When do I become eligible for the Texas esthetician exam?",
  ...
  {
    q: "Where can I check the official esthetician exam requirements?",
    a: "This page is transcribed from the PSI Esthetician Candidate Information Bulletin effective January 1, 2026. Each TDLR exam has its own bulletin — confirm yours at psiexams.com before your exam date.",
  },
];

export default function TexasEstheticianExamPrepPage() {
    ...
    <div className="min-h-screen light bg-white text-slate-950">
      ...
      <main className="mx-auto max-w-5xl px-4 sm:px-6 pt-28 pb-16">
        ...
        <div className="mb-16 rounded-2xl border border-slate-200 bg-slate-50 px-6 py-6">
          ...
          >
            View the esthetician kit list
          </Link>
        </div>

        <TdlrExamProcess courseName="Esthetician" hoursForWritten={900} />
        ...
        <div className="mb-10 rounded-2xl border border-slate-200 bg-slate-50 px-6 py-5 text-sm leading-relaxed text-slate-600">
          Transcribed from the PSI Esthetician Candidate Information Bulletin effective January 1, 2026 — the
      ...
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(graph(
            {
            "@type": "FAQPage",
            "@id": `${SITE_URL}/texas-esthetician-exam-prep#faqpage`,
      ...
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(graph(
            {
            "@type": "HowTo",
            "@id": `${SITE_URL}/texas-esthetician-exam-prep#howto`,
            "isPartOf": ref(WEBSITE_ID),
            "publisher": ref(ORG_ID), author: authorSchema(), name: "Texas Esthetician Practical Exam — Section Order", description: "The 8 timed sections of the Texas esthetician practical exam in order, per the PSI/TDLR Candidate Information Bulletin effective January 1, 2026.", totalTime: "PT1H41M", step: EXAM.sections.map((s, i) => ({ "@type": "HowToStep", position: i + 1, name: s.name, text: [...s.procedure, ...s.safety].join("; ") })) },
          )) }} />
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(graph(
            {
            "@type": "Course",
            "@id": `${SITE_URL}/texas-esthetician-exam-prep#course`,
            "isPartOf": ref(WEBSITE_ID),
            "publisher": ref(ORG_ID), name: "Texas Esthetician Exam Prep", description: 'Every scored criterion on the Texas esthetician practical exam, the 8 timed sections, and the written exam format — from the Jan 1, 2026 PSI/TDLR bulletin.', url: `${SITE_URL}/texas-esthetician-exam-prep`, provider: { "@type": "Organization", name: "Inner G Complete Agency", url: SITE_URL }, teaches: "Texas esthetician practical exam scored criteria, section order and written exam format", offers: { "@type": "Offer", price: "0", priceCurrency: "USD" } },
          )) }} />
    </div>
  );
}
